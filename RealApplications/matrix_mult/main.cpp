...
//DETERMING PREFETCHER DISTANCE BASED ON STATIC AND DYNAMIC FEATURES:
	hpx::parallel::for_each(hpx::parallel::execution::make_prefetcher_policy(policy, hpx::parallel::prefetching_distance_determination({hpx::get_os_thread_count(), 1223706, 320800, 10053, std::size_t(std::distance(time_range.begin(), time_range.end())), 2}), A, B, C), time_range.begin(), time_range.end(), f);
    ...
    //prefetching has +9 more instructions
    std::cout << std::left << (1.0E-09 * ((byte + 9) * sizeof(double)) / (elapsed_prefetch)) << " [GB/s]"; 
    std::cout << std::endl;
}
