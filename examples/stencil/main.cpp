    ...
    std::string const& output_name)
{
    ...
    for (int t = 0; t < steps; ++t)
    {
	    ...
	    hpx::parallel::for_loop(policy.with(hpx::parallel::adaptive_chunk_size()), begin, end, hpx::parallel::induction(next.middle + Nx, Nx), f);
    ...
    std::string const& output_name)
{
    ...
    for (std::size_t t = 0; t < steps; ++t)
    {
        ...
        // Update our interior spatial domain
	   hpx::parallel::for_each(hpx::parallel::execution::make_prefetcher_policy(policy, prefetch_distance_factor, U[0], U[1]), 
       begin, end, f);
    }    
    double elapsed = t.elapsed();
