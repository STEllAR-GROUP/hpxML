...
//DETERMING PREFETCHER DISTANCE BASED ON STATIC AND DYNAMIC FEATURES:
	hpx::parallel::for_each(hpx::parallel::execution::make_prefetcher_policy(policy, hpx::parallel::prefetching_distance_determination({hpx::get_os_thread_count(), 8, 8, 0, std::size_t(std::distance(original_range.begin(), original_range.end())), 0}), a, b, c), 
        ...
        // determined prefetching distance:
        prefetch_distance_factor = hpx::parallel::prefetching_distance_determination({hpx::get_os_thread_count(), 8, 8, 0, std::size_t(std::distance(original_range.begin(), original_range.end())), 0});
...
{    
        ...
        if(chunker == "dynamic")
        {
            ...
            workers.push_back(
                hpx::async(execs[i], &numa_domain_worker<vector_type, decltype(policy)>,
                    ...
                    vector_size, part_size, part_size*i, iterations, boost::ref(prefetch_distance_factor),
    ...
    // +30 instructions in calling prefetching
    const double bytes[3] = {
        7 * sizeof(STREAM_TYPE) * static_cast<double>(vector_size),
        7 * sizeof(STREAM_TYPE) * static_cast<double>(vector_size),
        (7 * sizeof(STREAM_TYPE) * static_cast<double>(vector_size) + 12 * sizeof(int) * static_cast<double>(vector_size) + 18 * sizeof(int))
    };
...
int main(int argc, char* argv[])
{
    ...
    cmdline.add_options()
        (   "vector_size",
            boost::program_options::value<std::size_t>()->default_value(50000000),
